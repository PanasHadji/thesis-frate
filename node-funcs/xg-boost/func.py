...
def xgboost(req):
    """
    Use XGBoost to classify a dataset read from MinIO and upload the results back to MinIO.
    """
    try:
...
def main(context):
    """
    Entry point for handling incoming HTTP requests for the xgboost function.
    """
    if 'request' in context.keys():
        if context.request.method == "POST":
            output_file = xgboost(context.request)
            print(output_file)
